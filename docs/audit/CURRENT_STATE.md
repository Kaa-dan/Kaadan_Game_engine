...
1. ~~**No scripting layer**~~ **Done in Phase 6** — `kaadan_script` provides Rust gameplay via dylib hot-reload (desktop) + static-link (mobile), a `ComponentRegistry`, and a `templates/game_template`. **The integrated code editor (P8) is still unstarted.**
...
- ~~"PBR" is Blinn-Phong.~~ **Fixed in Phase 4** (real Cook-Torrance GGX metallic-roughness + normal mapping + emissive). glTF import still drops textures — **moved to Phase 5** (needs the asset pipeline to manage texture handles).

**Performance risks (matter for the mobile/perf goal):**
- ~~PBR allocates per-entity uniform buffers + bind groups **every frame**.~~ **Fixed in Phase 4** (dynamic-offset uniform buffer + cached material bind groups; no steady-state GPU allocation).
...
- ~~Asset loading is fully synchronous (no async/streaming).~~ **Fixed in Phase 5** (`load_async` + background worker + `poll`).

**Dead / unused code & deps:**
- `Mesh` + `create_basic_pipeline` + `basic.wgsl` (unused colored-geometry path).
- ~~`LoadState::{Queued,Loading}` (never assigned).~~ **Phase 5**: `Queued` is now used by `load_async` (`Loading` intentionally left unused — documented).
- ~~`HotReloader` (orphaned — reports paths, reloads nothing).~~ **Phase 5**: wired via `AssetServer::reload_path` / `process_changes` (re-invokes the loader).
...
- **✅ Phase 6 — Scripting** (2026-05-30): new **`kaadan_script`** crate + **`templates/game_template`**. Gameplay is written in Rust against a `ScriptContext` facade (over `App`) and exported via the `kaadan_game!` macro as a stable `extern "C" fn kaadan_register(&mut ScriptContext)`. On **desktop**, `ScriptHost` `libloading`s the gameplay **cdylib** (copied to a temp file to avoid rebuild locks) and **hot-reloads** on file-mtime change — surgically removing the plugin's systems by name before unloading, so the host-owned `World`/`Resources` survive. On **mobile**, the same crate is **static-linked** (rlib) — iOS forbids loading user dylibs. Added a `ComponentRegistry` (name↔type-erased has/remove/insert_default) that also unblocks editor reflection (P7). `kaadan_ecs` gained `remove_system`. Safety contract documented in `docs/scripting/abi.md`. **Hot-reload verified manually** via an `#[ignore]`d integration test (builds the cdylib, loads, ticks, asserts) — the static-link path runs in CI. All green.
- **✅ Phase 5 — Asset pipeline (partial)** (2026-05-30): `kaadan_assets` gained **async, non-blocking loading** (`load_async` reserves a `Queued` handle, a dedicated background worker thread reads+decodes via closures, `poll()`/`update()` routes results into typed storage → `Loaded`/`Failed`); **functional hot-reload** (`reload_path` re-invokes the recorded loader and replaces the asset at the same handle with a version bump; `process_changes` integrates `HotReloader` under the `hot-reload` feature); and an **`AudioClip` + `AudioLoader`**. Synchronous `load` retained. No new external deps. **Deferred:** glTF texture loading + GPU-upload binding (couples to the engine + the in-flight editor) and on-disk processed cache. All green (assets 10 tests w/ feature).
- **✅ Phase 4 — 3D Renderer PBR + perf** (2026-05-30): rewrote `assets/shaders/pbr.wgsl` from Blinn-Phong to real **Cook-Torrance** metallic-roughness (GGX distribution, Smith-Schlick geometry, Fresnel-Schlick, F0/kS/kD energy split, dir + 4 point lights, optional albedo/MR/normal/emissive textures with tangent-space normal mapping, Reinhard tonemap); added `pbr_mobile.wgsl` / `PBR_MOBILE_SHADER`. `PbrRenderer` now binds material textures (with 1×1 white/normal/black defaults) and has **zero steady-state per-frame GPU allocation** — per-entity model+material uniforms live in growable dynamic-offset buffers and material-texture bind groups are cached by texture-handle set. Added naga WGSL **validation tests** for both shaders (`render` signature unchanged; call sites untouched). All green (59 tests). *Visual/lighting correctness not verified (headless).* glTF texture loading deferred to Phase 5.
